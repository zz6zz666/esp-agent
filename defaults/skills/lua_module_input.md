...
Keyboard and mouse input for interactive Lua scripts — the desktop
...
### `input.mouse_pos()`
Returns `x, y` — current mouse cursor position in logical display coords.
...
Returns the human-readable name for a scancode (e.g. 4 → `"A"`, 40 → `"Return"`).
