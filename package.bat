@echo off
REM package.bat — Package esp-agent for Windows distribution
REM
REM Creates a self-contained package with:
REM   esp-claw-desktop.exe  — the agent daemon
REM   esp-agent.exe         — the CLI management tool
...
set "VERSION=1.0.0"
set "PKG_NAME=esp-agent-windows-v%VERSION%"

echo === Packaging esp-agent v%VERSION% for Windows ===
...
copy /y "build\esp-agent.exe" "%PKG_NAME%\"
if errorlevel 1 (echo Failed to copy esp-agent.exe && exit /b 1)
...
if exist "packaging\usr\share\esp-agent\defaults\*" (
    xcopy /e /y "packaging\usr\share\esp-agent\defaults\*" "%PKG_NAME%\defaults\"
...
(
    echo esp-agent v%VERSION% for Windows
    echo ================================
    echo.
    echo Quick Start:
    echo   1. Run esp-agent.exe config   to configure LLM keys
    echo   2. Run esp-agent.exe start    to start the agent
    echo   3. Run esp-agent.exe ask "Hello"  to test
    echo.
    echo Files:
    echo   esp-claw-desktop.exe  — The agent daemon
    echo   esp-agent.exe         — CLI management tool
    echo   fonts/                — Font files for display rendering
    echo   defaults/             — Default configuration
    echo.
    echo Data is stored in: %%USERPROFILE%%\.esp-agent\
    echo.
