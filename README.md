...
Crush Agent 运行在你对象（或朋友、伴侣）的电脑上，你通过 AI 可以远程控制屏幕上的一只小龙虾 —— 让它绘制彩虹、告白动画、迷你游戏，或者只是静静地趴在那里陪伴。
...
**Crush Agent 将沙箱安全性作为第一要务。** 所有用户提交的 Lua 脚本都在一个深度隔离的虚拟环境中执行：
...
> ⚠️ **重要须知**：使用 Crush Agent 需要双方相互信任。你的 LLM API Key 和 Channel Token 会存储在对方的电脑上。请确保：
